...
import { Button, Group, Divider, Tooltip, Select, ActionIcon, Switch, Modal, TextInput, Stack, Text } from '@mantine/core';
...
export function WebContentEditor({ placeholder = 'Write your blog content…', initialHTML = '', onChangeHTML, defaultShowLabels = true, minRows = 10 }: Props) {
  ...
  const editor = useEditor({
    ...
    editorProps: {
      attributes: { class: 'webcontent-editor' },
      handleClick: (_view, _pos, event) => {
        const e = event as any;
        const target = (e?.target as HTMLElement) || null;
        if (target && target.closest('a')) {
          // Always prevent navigation; allow editor to place the caret
          e?.preventDefault?.();
          return false;
        }
        return false;
      ...
      handleDOMEvents: {
        mousedown: (_view: any, event: any) => {
          ...
          if (target && target.closest('a')) {
            // Block browser default on Ctrl/Cmd+Click as well
            event?.preventDefault?.();
            return false; // let editor handle caret placement
          }
          return false;
        },
      },
  ...
  // Re-render on selection changes so toolbar active states update
  useEffect(() => {
    if (!editor) return;
  ...
  const openLinkModal = () => {
    if (!editor) return;
    // Focus and expand to current link mark under the caret (if any)
