...
describe('services/employees helpers', () => {
  it('normalizeEmployee maps fields', () => {
    const raw: any = { name: 'User', email: 'U@Example.com', roleIds: ['r1'], permissionIds: ['p1'], isAdmin: true, isArchived: false, deletedAt: null, createdAt: 1, updatedAt: 2 };
    const v = normalizeEmployee('id1', raw);
    expect(v).toEqual({ id: 'id1', name: 'User', email: 'u@example.com', roleIds: ['r1'], permissionIds: ['p1'], isAdmin: true, isArchived: false, deletedAt: null, createdAt: 1, updatedAt: 2 });
  });
  ...
  it('buildEmployeePatch validates name/email', () => {
    ...
    const p = buildEmployeePatch({ name: ' N ', email: ' X@Y.Com ', isAdmin: true, isArchived: null, deletedAt: null });
    expect(p).toEqual({ name: 'N', email: 'x@y.com', isAdmin: true, isArchived: null, deletedAt: null });
  });
});
