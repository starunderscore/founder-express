...
describe('services/roles helpers', () => {
  it('normalizeRole handles missing/blank fields', () => {
    const raw: RawRoleDoc = { name: '  Name  ', permissionIds: ['a', null, 'b'], isArchived: false } as any;
    const r = normalizeRole('id1', raw);
    expect(r).toEqual({ id: 'id1', name: '  Name  ', description: undefined, permissionIds: ['a', 'b'], isArchived: false, deletedAt: undefined, createdAt: undefined });
  });

  it('roleStatus and roleBackLink map correctly', () => {
    const active: Pick<Role,'isArchived'|'deletedAt'> = { isArchived: false, deletedAt: undefined } as any;
    const archived: Pick<Role,'isArchived'|'deletedAt'> = { isArchived: true, deletedAt: undefined } as any;
    const removed: Pick<Role,'isArchived'|'deletedAt'> = { isArchived: false, deletedAt: Date.now() } as any;
  ...
  it('filterByStatus partitions correctly', () => {
    ...
    const rows: any[] = [
      ...
      { id: '3', isArchived: false, deletedAt: now },
    ];
