...
describe('ep_employees collection — admin-only access', () => {
  it('admin (owner) can CRUD; normal employee denied', async () => {
    ...
    const ref = await assertSucceeds(addDoc(collection(owner, 'ep_employees'), { name: 'A', email: 'a@example.com', isAdmin: false }));
    await assertSucceeds(getDocs(collection(owner, 'ep_employees')));
    await assertSucceeds(getDoc(doc(owner, 'ep_employees', (ref as any).id)));
    await assertSucceeds(updateDoc(doc(owner, 'ep_employees', (ref as any).id), { isAdmin: true }));
    await assertSucceeds(deleteDoc(doc(owner, 'ep_employees', (ref as any).id)));

    // Normal employee denied on list/get/create/update/delete
    await assertFails(getDocs(collection(emp, 'ep_employees')));
    await assertFails(getDoc(doc(emp, 'ep_employees', 'emp-1')));
    await assertFails(addDoc(collection(emp, 'ep_employees'), { name: 'X', email: 'x@example.com', isAdmin: false }));
});
});
