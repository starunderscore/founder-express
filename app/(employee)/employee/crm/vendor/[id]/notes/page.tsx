...
export default function VendorNotesPage({ params }: { params: { id: string } }) {
  ...
  const openEditNote = (id: string) => {
    const note = (vendor?.notes || []).find((n: any) => n.id === id);
    if (!note) return;
    setEditNoteId(id);
      ...
      <Card withBorder radius="md" padding={0}>
        ...
        <div style={{ padding: '12px 16px' }}>
          {Array.isArray(vendor.notes) && vendor.notes.length > 0 ? (
            <Stack>
              {vendor.notes.map((n: any) => (
                <Card key={n.id} withBorder radius="md" padding="sm">
                  <Group justify="space-between" align="flex-start">
                    <div>
                      <Group gap={8} align="center">
                        <Avatar size="sm" radius="xl" src={n.createdByPhotoURL} color="orange">
