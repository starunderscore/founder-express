...
import { Title, Text, Card, Stack, Group, Button, Tabs, Table, Menu, ActionIcon } from '@mantine/core';
import { useEffect, useState } from 'react';
import { listenEmailTemplates, restoreEmailTemplate, removeEmailTemplate, type EmailTemplateItem } from '@/lib/firebase/emailSettings';

export default function EmailTemplatesRemovedPage() {
  const router = useRouter();
  const [templates, setTemplates] = useState<EmailTemplateItem[]>([]);
  useEffect(() => { const off = listenEmailTemplates(setTemplates); return () => off(); }, []);
                    ...
                    <Table.Tr key={t.id}>
                      ...
                      <Table.Td>
                        <Menu withinPortal position="bottom-end" shadow="md" width={200}>
                          ...
                          <Menu.Dropdown>
                            <Menu.Item onClick={async () => { await restoreEmailTemplate(t.id); }}>Restore</Menu.Item>
                            <Menu.Item color="red" onClick={async () => { await removeEmailTemplate(t.id); }}>Delete permanently</Menu.Item>
                          </Menu.Dropdown>
                        </Menu>
                      </Table.Td>
                    </Table.Tr>
